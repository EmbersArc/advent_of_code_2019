...
fn intersect_lines(
    ...
    if (start2.x <= start1.x && start1.x <= end2.x) && (start1.y <= start2.y && end1.y >= start2.y)
    {
        return Some(Point {
            x: start1.x,
            y: start2.y,
...
fn actions_to_path(actions: &String) -> Vec<Point> {
    ...
    for action in result.iter() {
        ...
        let mut position = path.last().copied().unwrap();
...
fn main() {
    let first_path = actions_to_path(&get_line1());
    let second_path = actions_to_path(&get_line2());

    let mut intersections = Vec::new();
    let mut distances = Vec::new();

    let mut first_path_dist = 0;
    for i in 0..first_path.len() - 1 {
        let mut second_path_dist = 0;
        for j in 0..second_path.len() - 1 {
            if i == 0 && j == 0 {
                // not crossing at origin
                continue;
            }
            ...
            match intersection {
                Some(p) => {
                    ...
                    let first_dist_to_intersection = first_path[i].dist_to(p);
                    let second_dist_to_intersection = second_path[j].dist_to(p);

                    distances.push(
                        first_path_dist
                            + first_dist_to_intersection
                            + second_path_dist
                            + second_dist_to_intersection,
                    );
                }
                None => (),
            }
            second_path_dist += second_path[j + 1].dist_to(second_path[j]);
        }
        first_path_dist += first_path[i + 1].dist_to(first_path[i]);
    }
